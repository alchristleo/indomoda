    ...
    public partial class PenerimaanCMT : MetroFramework.Forms.MetroForm
    {
        ...
        private void btnResetPenerimaanCMT_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (dataGridView1.Rows.Count > 0 && (dba.qtyCMTBS == null && dba.qtyCMTHilang == null))
                {
            ...
            if (cboNoSpkCMTPenerimaan.Items.Count > 0 && cboNoSpkCMTPenerimaan.Text != "")
            {
                try
                {
                    ...
                    for (int i = 0; i < rowCount; i++)
                    {
                        ...
                        string noSeri = dataGridView1.Rows[i].Cells[1].Value.ToString();
                        int idSPKCMT = Convert.ToInt32(dataGridView1.Rows[i].Cells[18].Value.ToString());
                        var dba = GenericQuery.SqlQuerySingle<QRIDnoSeriModel>("SELECT qr.id, qr.noSeri, qr.qtyAwalSablon, qr.qtySablonBS, qr.qtySablonHilang, qr.qtyAwalBordir, qr.qtyBordirBS, qr.qtyBordirHilang, qr.qtyAwalCMT, qr.qtyCMTBS, qr.qtyCMTHilang, c.idSPKCMT FROM QuantityRecord qr JOIN ListPenerimaanTukangPotong c ON qr.noSeri = c.noSeri WHERE qr.noSeri = '" + noSeri + "' AND c.idSPKCMT = '" + idSPKCMT + "'");
                    ...
                    cboPicPenerimaanCMT.SelectedValue = dbb.EmployeeID;
                    var dbc = GenericQuery.SqlQuerySingle<Employee>("SELECT e.EmployeeID, e.EmployeeName, e.EmployeeCode, e.EmployeeEmail, e.EmployeePhone, e.EmployeePosition from Employees e WHERE e.EmployeeID = '" + cboPicPenerimaanCMT.SelectedValue + "'");
